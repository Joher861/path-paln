...
void AStar::initGridMap(const Eigen::Vector2i pool_size)
{
...
bool AStar::ConvertToIndexAndAdjustStartEndPoints(Vector2d start_pt, Vector2d end_pt, Vector2i &start_idx, Vector2i &end_idx)
{
    ...
    {
        TEB_DEBUG_LOG("Start point is insdide an obstacle.");
        do
        {
    ...
    {
        TEB_DEBUG_LOG("End point is insdide an obstacle.");
        do
        {
...
bool AStar::AstarSearch(const double step_size, Eigen::Vector2d start_pt, Eigen::Vector2d end_pt)
{
    uint64_t time_1 = Timer::getSystemTimestampMS();
    ++rounds_;
    ...
    {
        TEB_ERROR_LOG("Unable to handle the initial or end point, force return!");
        return false;
    }
    ...
    while (!openSet_.empty())
    {
        ...
        uint64_t time_2 = Timer::getSystemTimestampMS();
        if ((time_2 - time_1)*0.001 > 0.2)
        {
            TEB_WARN_LOG("Failed in A star path searching !!! 0.2 seconds time limit exceeded.");
            return false;
        }
    }
    ...
    if ((time_2 - time_1)*0.001 > 0.1)
        TEB_WARN_LOG("Time consume in A star path finding is %.3fs, iter=%d", (time_2 - time_1)*0.001, num_iter);
