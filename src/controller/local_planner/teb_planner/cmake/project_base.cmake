...
#			list: a b c
#
function(project_parse_function_args)
...
macro( add_to_project_include_dirs )
foreach( dir ${ARGN} )
  set_property( GLOBAL APPEND PROPERTY PROJECT_INCLUDE_DIRS "${dir}" )
  endforeach()
endmacro()


macro( add_to_project_libraries )
    foreach( lib ${ARGN} )
      ...
      if (TARGET ${lib})
          ...
          if (NOT ${is_lib_imported})
            set_property( GLOBAL APPEND PROPERTY PROJECT_LIBRARIES "${lib}" )
          else()
            # For imported targets, we just want to depend on the library directly
            get_target_property(libpath ${lib} LOCATION)
            if (libpath)
                set_property( GLOBAL APPEND PROPERTY PROJECT_LIBRARIES "${libpath}" )
...
macro( add_to_project_sources )
    ...
    foreach(_src ${ARGN})
        if(_relPath)
            set_property( GLOBAL APPEND PROPERTY PROJECT_SOURCES "${_relPath}/${_src}" )
        else()
            set_property( GLOBAL APPEND PROPERTY PROJECT_SOURCES "${_src}" )
        endif()
    endforeach()
endmacro()

macro( add_to_project_headers )
    if("${SDK_SOURCE_DIR}" STREQUAL "")
        file(RELATIVE_PATH _relPath "${CMAKE_SOURCE_DIR}" "${CMAKE_CURRENT_SOURCE_DIR}")
    else()
        file(RELATIVE_PATH _relPath "${SDK_SOURCE_DIR}" "${CMAKE_CURRENT_SOURCE_DIR}")
    ...
    foreach(_hdr ${ARGN})
        if(_relPath)
            set_property( GLOBAL APPEND PROPERTY PROJECT_HEADERS "${_relPath}/${_hdr}" )
        else()
            set_property( GLOBAL APPEND PROPERTY PROJECT_HEADERS "${_hdr}" )
        endif()
    endforeach()
endmacro()

macro( project_set_compile_flags file flags )
...
#=============================================================================
#
#	project_strip_optimization
#
function(project_strip_optimization name)
	set(_compile_flags)
...
#
#	Usage:
#		project_join(OUT ${OUT} [ LIST ${LIST} ] GLUE ${GLUE})
#
#	Input:
#		LIST		: list to join
...
#
#	Example:
#		project_join(OUT test_join LIST a b c GLUE ";")
...
function(project_add_module)
        ...
        project_add_library(${MODULE} STATIC EXCLUDE_FROM_ALL ${SRCS})

        # Pass variable to the parent project_add_module.
        ...
        foreach(prop LINK_FLAGS)
                if(${prop})
                        project_join(OUT ${prop} LIST ${${prop}} GLUE " ")
                endif()
        endforeach()

        if(COMPILE_FLAGS AND ${_no_optimization_for_target})
             project_strip_optimization(COMPILE_FLAGS ${COMPILE_FLAGS})
        endif()
        foreach (prop LINK_FLAGS )
           if (${prop})
	...
	set_property(GLOBAL APPEND PROPERTY PROJECT_LIBRARIES ${MODULE})
        set_property(GLOBAL APPEND PROPERTY PROJECT_MODULE_PATHS ${CMAKE_CURRENT_SOURCE_DIR})
endfunction()

#=============================================================================
#
#	project_add_optimization_flags_for_target
#
set(all_posix_cmake_targets "" CACHE INTERNAL "All cmake targets for which optimization can be suppressed")
function(project_add_optimization_flags_for_target target)
	...
	# Pass variable to the parent project_add_library.
	set(_no_optimization_for_target ${_no_optimization_for_target} PARENT_SCOPE)
endfunction()
...
function(project_add_executable target)
	...
	project_add_optimization_flags_for_target(${target})
endfunction()
...
function(project_add_library target)
	...
	set_property(GLOBAL APPEND PROPERTY PROJECT_LIBRARIES ${target})
        set_property(GLOBAL APPEND PROPERTY PROJECT_MODULE_PATHS ${CMAKE_CURRENT_SOURCE_DIR})
...
#
#	Example:
#		project_prebuild_targets(OUT target_list BOARD r16)
#
function(project_prebuild_targets)
        project_parse_function_args(
                        NAME project_prebuild_targets
                        ONE_VALUE OUT BOARD
                        REQUIRED OUT
                        ARGN ${ARGN})
