...
void SDFMap::initMap(double size_x,double size_y,double res,double inflation_dis) {
  /* get parameter */
  double x_size, y_size, z_size;
  mp_.resolution_ = res;
  x_size = size_x * res;//unit:m
  y_size = size_x * res;
  z_size = res;
  mp_.local_update_range_(0) = x_size * 0.5;
  mp_.local_update_range_(1) = y_size * 0.5;
  mp_.local_update_range_(2) = res;
  mp_.obstacles_inflation_ = inflation_dis;
  ...
  mp_.visualization_truncate_height_ = 2.49;
  mp_.virtual_ceil_height_ = 2.45;

  mp_.show_occ_time_ = false;
  mp_.show_esdf_time_ = false;
  mp_.pose_type_ = 1;

  mp_.local_bound_inflate_ = 1.0;
  mp_.local_map_margin_ = 1;
...
void SDFMap::setMapOrigin(double x,double y)
{
  mp_.map_origin_ = Eigen::Vector3d(x - mp_.map_size_(0) / 2.0, y - mp_.map_size_(1) / 2.0, mp_.ground_height_);
  mp_.map_min_boundary_ = mp_.map_origin_;
  mp_.map_max_boundary_ = mp_.map_origin_ + mp_.map_size_;

  LMAP_DEBUG_LOG("map_origin{%f,%f,%f} map_min_boundary{%f,%f,%f} map_max_boundary{%f,%f,%f}",
    mp_.map_origin_(0),mp_.map_origin_(1),mp_.map_origin_(2),mp_.map_min_boundary_(0),
    mp_.map_min_boundary_(1),mp_.map_min_boundary_(2),mp_.map_max_boundary_(0),
    mp_.map_max_boundary_(1),mp_.map_max_boundary_(2));
}
...
//     LMAP_WARN_LOG("Fusion: cur t = %lf, avg t = %lf, max t = %lf", (t2 - t1).toSec(),
...
void SDFMap::updateESDFCallback() {
  ...
  t2 = Timer::getSystemTimestampMS();

  md_.esdf_time_ += (t2 - t1) * 0.001f;
  md_.max_esdf_time_ = max(md_.max_esdf_time_, (t2 - t1) * 0.001);

  if (mp_.show_esdf_time_)
    LMAP_WARN_LOG("ESDF: cur t = %lf, avg t = %lf, max t = %lf", (t2 - t1) * 0.001,
...
void SDFMap::odomCallback(float x,float y) {
  if (md_.has_first_depth_) return;

  md_.camera_pos_(0) = x;
  md_.camera_pos_(1) = y;
  md_.camera_pos_(2) = mp_.ground_height_;

  md_.has_odom_ = true;
}

void SDFMap::cloudCallback(const std::vector<Eigen::Vector2d>& cloud) {
  ...
  this->resetBuffer(md_.camera_pos_ - mp_.local_update_range_,
                    md_.camera_pos_ + mp_.local_update_range_);//复位选定区域
  ...
  for (size_t i = 0; i < cloud.size(); ++i) {
    pt = cloud[i];
    p3d(0) = pt(0), p3d(1) = pt(1), p3d(2) = mp_.ground_height_;
      ...
      for (int x = -inf_step; x <= inf_step; ++x)
        for (int y = -inf_step; y <= inf_step; ++y)
          for (int z = -inf_step_z; z <= inf_step_z; ++z) {
            ...
            p3d_inf(0) = pt(0) + x * mp_.resolution_;
            p3d_inf(1) = pt(1) + y * mp_.resolution_;
            p3d_inf(2) = mp_.ground_height_ + z * mp_.resolution_;
  ...
  LMAP_INFO_LOG("min_x=%lf min_y=%lf min_z=%lf max_x=%lf max_y=%lf max_z=%lf",min_x,min_y,min_z,max_x,max_y,max_z);
  min_x = min(min_x, md_.camera_pos_(0));
  min_y = min(min_y, md_.camera_pos_(1));
  min_z = min(min_z, md_.camera_pos_(2));
