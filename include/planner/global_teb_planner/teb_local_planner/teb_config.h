...
{
#define CONFIG_TEB_PLANNER    "cfg_Teb_planner"
DEFINE_CONFIG(TebPlanner)
    int ev_deduction_period_for_us; // event deduction period for emergency stop/slowdown
END_CONFIG(TebPlanner)

#define CONFIG_TRAJECTORY	"cfg_TebTrajectory"
DEFINE_CONFIG(TebTrajectory)
    ...
    int no_viapoint_idx;//!< When adding viapoints, Ignore the first few points in transform plan  
...
END_CONFIG(TebTrajectory)

#define CONFIG_TEBROBOT	"cfg_TebRobot"
DEFINE_CONFIG(TebRobot)
    double max_vel_x; //!< Maximum translational velocity of the robot
    double min_vel_x; //!< Maximum translational velocity of the robot
...
#define CONFIG_GOALTOLERANCE	"cfg_TebGoalTolerance"
DEFINE_CONFIG(TebGoalTolerance)
    ...
    double controller_patience; //无效控制的超时时间
END_CONFIG(TebGoalTolerance)

#define CONFIG_OBSTACLE	"cfg_TebObstacle"
DEFINE_CONFIG(TebObstacle)
    ...
    double obstacle_angle_increasment;//添加障碍物的角度增量
    ...
    bool use_sdf_map;
END_CONFIG(TebObstacle)

#define CONFIG_OPTIMIZATION	"cfg_TebOptimization"
DEFINE_CONFIG(TebOptimization)
...
END_CONFIG(TebOptimization)

#define CONFIG_HC	"cfg_TebHomotopyClasses"
DEFINE_CONFIG(TebHomotopyClasses)
    ...
    int simple_exploration; //!< If true, distinctive trajectories are explored using a simple left-right approach (pass each obstacle on the left or right side) for path generation, otherwise sample possible roadmaps randomly in a specified region between start and goal.
    int max_number_classes; //!< Specify the maximum number of allowed alternative homotopy classes (limits computational effort)
    bool selection_enable_same_orientation; //!< Specify weather we perfer the trajectory which has simalar orientation with last best optimized teb
    double selection_same_orientation_tolerance; //!< Specify proper angle diff for ajuging if trajectories have the same orientation with last best optimized teb.
    double selection_same_orientation_scale; //!< Scale the cost of trajectories have the same orientation with last best optimized teb.
    ...
    int selection_alternative_time_cost; //!< If true, time cost is replaced by the total transition time.
    double selection_kinematics_cost_scale; //!< If true, time cost is replaced by the total transition time.
    ...
    bool use_astar; //!< Detours are discarted if their execution time / the execution time of the best teb is > this
END_CONFIG(TebHomotopyClasses)

#define CONFIG_RECOVERY	"cfg_TebRecovery"
DEFINE_CONFIG(TebRecovery)
...
END_CONFIG(TebRecovery)

#define CONFIG_FOOTSTR	"cfg_TebFootstr"
DEFINE_CONFIG(TebFootstr)
	std::string foot_str;
END_CONFIG(TebFootstr)

#define CONFIG_TEBLOG	"cfg_Teblog"
DEFINE_CONFIG(Teblog)
    std::string log_name;
    std::string log_path;
...
#define CONFIG_TEBTURNPID	"cfg_TebTurnPid"
DEFINE_CONFIG(TebTurnPid)
    float turn_p;
    float turn_i;
    float turn_d;
    float turn_scale;
    float turn_min_error;
    float turn_min_output;
    float turn_max_output;
    float turn_error_inc;
    bool turn_continuous_angle;
END_CONFIG(TebTurnPid)
...
public:
  ...
  ConfigTebTrajectory trajectory;
  ConfigTebRobot robot;
  ConfigTebGoalTolerance goal_tolerance;
  ConfigTebObstacle obstacles;
  ConfigTebOptimization optim;
  ConfigTebHomotopyClasses hcp;
  ConfigTebRecovery recovery;
  ConfigTebFootstr footprint;
  ...
  */
   ...
   */
  void reconfigure(TebConfig& cfg);
