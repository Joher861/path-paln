...
private:
	...
	inline bool checkOccupancy(const Eigen::Vector2d &pos) { return (bool)g_local_map.getInflateOccupancy(pos); }
...
public:
	...
	void initGridMap(const Eigen::Vector2i pool_size);
...
inline bool AStar::Coord2Index(const Eigen::Vector2d &pt, Eigen::Vector2i &idx) const
{
	...
	{
		TEB_ERROR_LOG("Ran out of pool, index=%d %d ", idx(0), idx(1));
		return false;
	}
