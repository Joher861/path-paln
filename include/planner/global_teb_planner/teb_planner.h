#ifndef TEB_GLOBAL_PLANNER_ROS_H_
#define TEB_GLOBAL_PLANNER_ROS_H_
...
#include "teb_local_planner/SimPID.h"

#include <teb_local_planner/obstacles.h>
#include <teb_local_planner/teb_config.h>
#include <teb_local_planner/angles.h>
#include <teb_planner_events.h>
...
#include <base_local_planner/tf/tf.h>
#include <base_local_planner/tf/transform_listener.h>
...
#include "teb_local_planner/dyn_a_star.h"
...
namespace planning_planner
{
  ...
  */
  class GTebPlanner : public GlobalPlanner
  {
    struct RotationInfo
    {
      bool self_rotation_enable;
      bool self_rotation_dir;
   ...
   */
    ...
    */
    bool pruneGlobalPlan(const RobotPose &global_pose,
                         std::vector<geometry_msgs::PoseStamped> &global_plan, int *current_goal_idx, double dist_behind_robot = 1);
...
private:
  ...
  virtual void finishPlanning() override;
  virtual bool handleInput(const GlobalPlannerInputPtr input) override;
  ...
  float controller_patience_ = 10;//单位：秒  该时间段内获取不到有效执行速度，可重新尝试局部规划；
                                  //超时将请求全局规划。
  TebPlannerInput::RunState inite_state_ = TebPlannerInput::ONCONTROLLING;//状态机初始运行状态
  ...
  TebPlannerInput::RecoveryType recovery_state_ = TebPlannerInput::MAX_RECOVERY_TYPE;
  ...
  void setRecoveryState(TebPlannerInput::RecoveryType state);
  ...
  shared_recursive_mutex rotation_mtx_;
  shared_recursive_mutex velocity_mtx_;
  VelocityInfo velocity_info_;
  ...
  bool global_path_first_ = false;
  bool is_global_path_valid_ = false;
  bool is_global_path_end_ = false;
  bool is_robot_as_start_ = false;

  AStar::Ptr a_star_;
  bool try_astar_path_ = false;
  bool local_path_collide_ = false;
  bool start_path_collide_check_ = false;
...
extern planning_planner::GTebPlanner g_teb_planner;
